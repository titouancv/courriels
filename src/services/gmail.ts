...
export function getAttachments(payload: any): Attachment[] {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const contentId = headers.find((h: any) => h.name === 'Content-ID' || h.name === 'X-Attachment-Id')?.value?.replace(/[<>]/g, '');
...
export async function fetchThreadsList(accessToken: string, query: string, pageToken?: string) {
...
export async function fetchThreadDetails(accessToken: string, threadId: string): Promise<Email> {
  ...
  const messages = threadData.messages.map((msgData: any) => {
    ...
    const date = new Date(getHeader('Date') || parseInt(msgData.internalDate));
  ...
  return {
    ...
    labels: labelIds.filter((l: string) => !['INBOX', 'UNREAD', 'IMPORTANT', 'CATEGORY_PERSONAL', 'CATEGORY_UPDATES', 'CATEGORY_FORUMS', 'CATEGORY_PROMOTIONS'].includes(l)),
    folder: folder,
  } as Email;
}

export async function fetchAttachmentData(accessToken: string, messageId: string, attachmentId: string): Promise<string | null> {
  try {
...
export async function sendEmailMessage(accessToken: string, to: string, subject: string, body: string, threadId?: string, inReplyTo?: string, references?: string) {
  const messageParts = [
    `To: ${to}`,
